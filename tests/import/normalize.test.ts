...
describe('parseDelimited', () => {
  it('splits on comma, trims, and drops empty pieces', () => {
    expect(parseDelimited('Male Artists, Female Artists')).toEqual([
      'Male Artists',
      'Female Artists',
    ])
...
describe('parseDateOrNull', () => {
  it('passes through a full ISO date unchanged with no warning', () => {
    expect(parseDateOrNull('2025-07-27', 'Date Added')).toEqual({
      value: '2025-07-27',
      warning: null,
    })
  })
  ...
  it('returns null with no warning for an empty value', () => {
    expect(parseDateOrNull('', 'Date Added')).toEqual({
      value: null,
      warning: null,
    })
    expect(parseDateOrNull(undefined, 'Date Added')).toEqual({
      value: null,
      warning: null,
    })
  })
  ...
  it('flags an unparseable date', () => {
    ...
    expect(result.warning).toEqual({
      field: 'Date Added',
      reason: 'unparseable date "not a date"',
    })
  })
})
...
describe('parseTimestampOrNull', () => {
  ...
  it('returns null with no warning for an empty value', () => {
    expect(parseTimestampOrNull('', 'Last Updated')).toEqual({
      value: null,
      warning: null,
    })
  })
...
describe('parseNumericOrNull', () => {
  it('passes through a valid number as a string', () => {
    expect(parseNumericOrNull('2500.0', 'Purchase Price')).toEqual({
      value: '2500.0',
      warning: null,
    })
  })
  ...
  it('flags an unparseable number', () => {
    ...
    expect(result.warning).toEqual({
      ...
      reason: 'unparseable number "abc"',
    })
  })

  it('returns null with no warning for an empty value', () => {
    expect(parseNumericOrNull('', 'Purchase Price')).toEqual({
      value: null,
      warning: null,
    })
  })
...
describe('parseCreationYear', () => {
  ...
  it('flags a value with no leading year', () => {
    ...
    expect(result.warning).toEqual({
      field: 'Creation Date',
      reason: 'unparseable creation date "circa"',
    })
  })
...
describe('parseBirthDate', () => {
  it('approximates a bare year to January 1st, with a warning', () => {
    ...
    expect(result.warning).toEqual({
      field: 'Birth Date',
      reason: 'bare year "1974" approximated to January 1',
    })
  })

  it('passes through a full ISO date unchanged with no warning', () => {
    expect(parseBirthDate('1976-05-02')).toEqual({
      value: '1976-05-02',
      warning: null,
    })
  })
  ...
  it('flags an unparseable birth date', () => {
    ...
    expect(result.warning).toEqual({
      field: 'Birth Date',
      reason: 'unparseable birth date "sometime in the 70s"',
    })
  })
})
