...
async function cleanupArtwork(sourcePieceId: number): Promise<void> {
  ...
  await testDb
    .delete(artworkFiles)
    .where(eq(artworkFiles.artworkId, artwork.id))
  await testDb
  ...
  it('imports every scenario correctly and is safe to run twice', async () => {
    ...
    const contactsCsv =
      [
        ...
        contactsRow({
          sourceContactId: gallerySourceId,
          companyName: 'Test Gallery',
        }),
        contactsRow({
          sourceContactId: sellerSourceId,
          companyName: 'Seller Gallery LLC',
        }),
    ...
    const piecesCsv =
      [
        ...
        piecesRow({
          ...
          additionalImages: [
            {
              url: 'https://example.com/additional-1.jpg',
              caption: 'side view',
            },
          ],
    ...
    try {
      ...
      const [ada] = await testDb
        .select()
        .from(contacts)
        .where(eq(contacts.sourceContactId, adaSourceId))
      const [charles] = await testDb
        .select()
        .from(contacts)
        .where(eq(contacts.sourceContactId, charlesSourceId))
      expect(artistLinks.find((l) => l.contactId === ada.id)?.role).toBe(
        'primary',
      )
      expect(artistLinks.find((l) => l.contactId === charles.id)?.role).toBe(
        'additional',
      )
      ...
      const images = await testDb
        .select()
        .from(artworkImages)
        .where(eq(artworkImages.artworkId, multiImage.id))
      expect(images).toHaveLength(2)
      expect(images.find((i) => i.isPrimary)?.sourceUrl).toBe(
      ...
      const files = await testDb
        .select()
        .from(artworkFiles)
        .where(eq(artworkFiles.artworkId, multiFile.id))
      expect(files).toHaveLength(2)
      expect(files.find((f) => f.name === 'Front')?.sourceUrl).toBe(
        'https://example.com/front.jpg',
      )
      expect(files.find((f) => f.name === 'Back')?.sourceUrl).toBe(
      ...
      const [blank] = await testDb
        .select()
        .from(artworks)
        .where(eq(artworks.sourcePieceId, blankPieceId))
      expect(
        await testDb
          .select()
          .from(artworkImages)
          .where(eq(artworkImages.artworkId, blank.id)),
      ).toHaveLength(0)
      expect(
        await testDb
          .select()
          .from(artworkFiles)
          .where(eq(artworkFiles.artworkId, blank.id)),
      ).toHaveLength(0)
      ...
      const [seller] = await testDb
        .select()
        .from(contacts)
        .where(eq(contacts.sourceContactId, sellerSourceId))
      ...
      expect(
        summary.warningMessages.some((m) =>
          m.includes('artist id 999999999 not found in contacts'),
        ),
      ).toBe(true)
      ...
      const contactRows = await testDb
        .select()
        .from(contacts)
        .where(eq(contacts.sourceContactId, adaSourceId))
      ...
      const imagesAfter = await testDb
        .select()
        .from(artworkImages)
        .where(eq(artworkImages.artworkId, multiImage.id))
      expect(imagesAfter).toHaveLength(2)

      const filesAfter = await testDb
        .select()
        .from(artworkFiles)
        .where(eq(artworkFiles.artworkId, multiFile.id))
      ...
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, gallerySourceId))
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, sellerSourceId))
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, blankSourceId))
      await testDb
        .delete(collections)
