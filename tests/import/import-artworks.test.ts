...
describe('importArtworks', () => {
  it('inserts an artwork with a resolved seller, two artists, an image, a file, a collection, and a tag', async () => {
    ...
    try {
      ...
      const [artwork] = await testDb
        .select()
        .from(artworks)
        .where(eq(artworks.sourcePieceId, record.sourcePieceId))
      ...
      expect(artistLinks.find((l) => l.contactId === primary.id)?.role).toBe(
        'primary',
      )
      expect(artistLinks.find((l) => l.contactId === additional.id)?.role).toBe(
        'additional',
      )
    } finally {
      const [artwork] = await testDb
      ...
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, primarySourceId))
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, additionalSourceId))
      await testDb
        .delete(contacts)
  ...
  it('warns and leaves sellerContactId null when the seller contact is not found', async () => {
    const record = baseArtworkRecord({
      title: 'Orphan Seller Piece',
      sellerSourceContactId: 999_999_999,
    })
    ...
    try {
      ...
      expect(result.warningMessages).toEqual([
        `row ${record.rowNumber}: seller contact id 999999999 not found`,
      ])
      const [artwork] = await testDb
        .select()
        .from(artworks)
  ...
  it('warns and drops an unresolvable artist id while keeping resolvable ones', async () => {
    ...
    try {
      const result = await importArtworks(
        testDb,
        [record],
        new Map([[knownSourceId, known.id]]),
      )

      expect(result.artworkArtists.processed).toBe(1)
      expect(result.warningMessages).toEqual([
        `row ${record.rowNumber}: artist id 999999999 not found in contacts`,
      ])
      const [artwork] = await testDb
        .select()
        .from(artworks)
      ...
      await testDb
        .delete(contacts)
        .where(eq(contacts.sourceContactId, knownSourceId))
    }
  })
  ...
  it('is idempotent: importing the same record twice does not duplicate rows', async () => {
    ...
    const record = baseArtworkRecord({
      ...
      images: [
        {
          url: 'https://example.com/idempotent.jpg',
          caption: null,
          sortOrder: 0,
          isPrimary: true,
        },
    ...
    try {
      ...
      const imageRows = await testDb
        .select()
        .from(artworkImages)
        .where(eq(artworkImages.artworkId, rows[0].id))
      expect(imageRows).toHaveLength(1)

      const tagRows = await testDb
        .select()
        .from(artworkTags)
        .where(eq(artworkTags.artworkId, rows[0].id))
