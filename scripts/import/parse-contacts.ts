...
export function parseContactsRows(rows: string[][]): ParseContactsResult {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (sourceContactIdRaw === '' || !Number.isFinite(sourceContactId)) {
      skipped.push({
        row: rowNumber,
        reason: `missing/invalid Contact Id "${sourceContactIdRaw}"`,
      })
      continue
    }
    ...
    if (birthDate.warning)
      warnings.push({ row: rowNumber, reason: birthDate.warning.reason })

    const deathDate = parseDateOrNull(row[15], 'Death Date')
    if (deathDate.warning)
      warnings.push({ row: rowNumber, reason: deathDate.warning.reason })

    const dateAdded = parseDateOrNull(row[row.length - 1], 'Date Added')
    if (dateAdded.warning)
      warnings.push({ row: rowNumber, reason: dateAdded.warning.reason })

    const sourceLocationIdRaw = row[37]?.trim() ?? ''
    const sourceLocationId =
      sourceLocationIdRaw !== '' && Number.isFinite(Number(sourceLocationIdRaw))
